def import_fixtures(self):
        ...
        self.redis.lpush('cuisine_template', '{%if params.method%}{{params.method}} {%endif%}{%if params.flavor%}{{params.flavor}} {%endif%}{{params.dish}}{%if params.mealtype%} {{params.mealtype}}{%endif%}{%if params.sauce%} in {{params.sauce}} {{params.saucetype}}{%endif%}{%if params.served%}, served {{params.served}}{%endif%} This dish is {{params.rarity["name"]}} to the {{params.region.name["full"]}}. Travelers consider the dish {{params.spice["name"]}} and {{params.presentation["name"]}} to the eye. Portions are usually {{params.size["name"]}}.')
