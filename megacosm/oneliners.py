...
def generatedrink():
    ...
    if ('count' in request.args and
        ...
        drinks = Drink(app.config['REDIS'], features)
...
def generatebond():
    ...
    features['npc'] = NPC(app.config['REDIS'], {})
    if 'count' in request.args and valid_count(request.args['count']):
        bonds = []
        for _ in range(int(request.args['count'])):
            bonds.append(Bond(app.config['REDIS'], features))
            features['seed'] = set_seed()
        return render_template('oneliner.html', oneliners=bonds,
                               oneliner=bonds[0], titletext=titletext, generator='bond')
    else:
        bond = Bond(app.config['REDIS'], features)
...
def bond_builder():
    ...
    for stat in ['when', 'template']:
        statinfo[stat] = []
        for statstring in app.config['REDIS'].lrange('bond_'+stat, 0, -1):
...
def generatejobposting():
    ...
    if 'count' in request.args and valid_count(request.args['count']):
        jobpostings = []
        for _ in range(int(request.args['count'])):
            jobpostings.append(JobPosting(app.config['REDIS'], features))
            features['seed'] = set_seed()
        ...
        jobposting = JobPosting(app.config['REDIS'], features)
...
def generatemotivation():
    ...
    if 'count' in request.args and valid_count(request.args['count']):
        motivations = []
        for _ in range(int(request.args['count'])):
            motivations.append(Motivation(app.config['REDIS'], features))
            features['seed'] = set_seed()
        ...
        motivation = Motivation(app.config['REDIS'], features)
...
def generatemundaneitem():
    ...
    if 'count' in request.args and valid_count(request.args['count']):
        mundaneitems = []
        for _ in range(int(request.args['count'])):
            mundaneitems.append(MundaneItem(app.config['REDIS'], features))
            features['seed'] = set_seed()
        ...
        mundaneitem = MundaneItem(app.config['REDIS'], features)
